...
@media (max-width: 600px) {
  ...
  html += "    </p>\r\n";
  html += "    <p>\r\n";
  html += "      <input type=\"text\" id=\"broker\" name=\"broker\" placeholder=\"Ip de la consola\" required><i class=\"validation\"><span></span><span></span></i>\r\n";
  html += "    </p>\r\n";
  html += "    <p>\r\n";
  html += "      <input type=\"number\" id=\"port\" name=\"port\" placeholder=\"Puerto\" required><i class=\"validation\"><span></span><span></span></i>\r\n";
  html += "    </p>\r\n";
  html += "    <p>\r\n";
  html += "      <button type=\"button\" id=\"Conectar\" value=\"Conectar\">Conectar</button>\r\n";
  html += "    </p>\r\n";
  html += "<div id=\"log\"  ></div>\r\n";
  html += "  </form>\r\n";
  html += "</div>\r\n";
  ...
  html += "  });\r\n";
  html += "});\r\n";
  html += "document.getElementById(\"Conectar\").addEventListener(\"click\", function() {\r\n";
  html += "  const ssid = document.getElementById('networks');\r\n";
  html += "  const password = document.getElementById('password');\r\n";
  html += "  const broker = document.getElementById('broker');\r\n";
  html += "  const port = document.getElementById('port');\r\n";
  html += "  const formData = new FormData();\r\n";
  html += "  formData.append('ssid', ssid.value);\r\n";
  html += "  formData.append('password', password.value);\r\n";
  html += "  formData.append('broker', broker.value);\r\n";
  html += "  formData.append('port', port.value);\r\n";
  html += "  fetch('/save', {\r\n";
  html += "    method: 'POST',\r\n";
  html += "    body: formData\r\n";
  html += "  })\r\n";
  ...
  html += "      } else {\r\n";
  html += "        throw new Error('Error en la solicitud');\r\n";
  html += "      }\r\n";
  html += "    })\r\n";
  html += "    .then(data => {\r\n";
  html += "      var message = document.getElementById('log');\r\n";
  html += "      message.innerHTML = data;\r\n";
  html += "    })\r\n";
  html += "    .catch(error => {\r\n";
  html += "      console.error('Error:', error);\r\n";
  html += "      var message = document.getElementById('log');\r\n";
  html += "      message.innerHTML = 'Error: ' + error;\r\n";
  html += "    });\r\n";
  html += "});\r\n";
  html += "</script>\r\n";
  html += "</body>\r\n";
...
void handleSave()
{
  ...
  docmqtt["BrokerAddress"] = server.arg("broker");
  docmqtt["Port"] = server.arg("port");
  ...
  server.send(200, "text/html", "Los datos se grabaron exitosamente, el equipo se reiniciara y se conectara a la red ");
  delay(1000);
  ESP.restart();
}
